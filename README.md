## ⋅⋆─────────⋆⋅Hello and Welcome⋅⋆───────⋆⋅
<!--
**lentilinternational/lentilinternational** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->

☆ I’m currently working on creating a multi project godot collection.<br>
☆ [Indie Games](https://9fold.itch.io/) <br>
☆ How to reach me: lentilinternational@gmail.com<br>
☆ Pronouns: any/all<br>
☆ Fun fact: I have only just started my programming journey! I aim to be a jack of all trades down the line.<br>

## ⋅⋆─────────⋆⋅Programming Languages⋅⋆───────⋆⋅
☆ HTML<br>
☆ CSS<br>
☆ Basic Python<br>
☆ PHP + MySQL<br>
☆ Ruby<br>
☆ C#<br>
☆ Javascript<br>
☆ GDScript<br>
☆ Advanced Python (Currently Learning) <br>

## ⋅⋆─────────⋆⋅Tool Usage⋅⋆───────⋆⋅
☆ Photoshop<br>
☆ Blender<br>
☆ Maya<br>
☆ Notepad++<br>
☆ VSCode<br>
☆ Excel<br>
☆ Microsoft Suite<br>
☆ Google Suite<br>
☆ Godot<br>

## ⋅⋆─────────⋆⋅VSCode Setup⋅⋆───────⋆⋅
**Theme:** Dracula<br>
**Font:** Fira Code (with ligatures)<br>
**Icons:** Material Icon Theme<br>

Extension⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀Description<br>
☆ Dracula Theme ─────── A dark, high-contrast theme.<br>
☆ Code Runner ──────── Run code snippets quickly inside VS Code.<br>
☆ Indent Rainbow ────── Adds color to indentation levels.<br>
☆ Material Icon Theme ─── Beautiful file icons for improved navigation.<br>

## ⋅⋆─────────⋆⋅Final Notes⋅⋆───────⋆⋅
☆ snake_case for life
